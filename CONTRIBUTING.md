# Contributing to Arcus Code

We want to make it easy for you to contribute to Arcus Code. Here are the most common type of changes that get merged:
...
- [`help wanted`](https://github.com/william-mckeon/arcus-code/issues?q=is%3Aissue%20state%3Aopen%20label%3Ahelp-wanted)
- [`good first issue`](https://github.com/william-mckeon/arcus-code/issues?q=is%3Aissue%20state%3Aopen%20label%3A%22good%20first%20issue%22)
- [`bug`](https://github.com/william-mckeon/arcus-code/issues?q=is%3Aissue%20state%3Aopen%20label%3Abug)
- [`perf`](https://github.com/william-mckeon/arcus-code/issues?q=is%3Aopen%20is%3Aissue%20label%3A%22perf%22)
...
By default, `bun dev` runs Arcus Code in the `packages/opencode` directory. To run it against a different directory or repository:

```bash
bun dev <directory>
```

To run Arcus Code in the root of this repository itself:
...
```bash
./packages/opencode/dist/arcus-code-<platform>/bin/arcus-code
```

Replace `<platform>` with your platform (e.g., `darwin-arm64`, `linux-x64`).

- Core pieces:
  - `packages/opencode`: CLI, server and business logic. Published as `arcus-code`.
...
# Production
arcus-code --help          # Show all available commands
arcus-code serve           # Start headless API server
arcus-code web             # Start server + open web interface
arcus-code <directory>     # Start TUI in specific directory
```
...
The most reliable way to debug Arcus Code is to run it manually in a terminal via `bun run --inspect=<url> dev ...` and attach
your debugger via that URL. Other methods can result in breakpoints being mapped incorrectly, at least in VSCode (YMMV).

Caveats:

- If you want to run the Arcus Code TUI and have breakpoints triggered in the server code, you might need to run `bun dev spawn` instead of
...
- The same problem arises when running Arcus Code in the VSCode `JavaScript Debug Terminal`
...
For net-new functionality, start with a design conversation. Open an issue describing the problem, your proposed approach (optional), and why it belongs in Arcus Code. The core team will help decide whether it should move forward; please wait for that approval instead of opening a feature PR directly.
