...
// mcp add creates the highest-precedence Arcus config when none exists.
const GLOBAL_CONFIG = Brand.configFiles[0]
    ...
    ({ home, opencode }) =>
      Effect.gen(function* () {
        ...
        const config = yield* Effect.promise(() =>
          Bun.file(path.join(home, ".config", Brand.slug, GLOBAL_CONFIG)).json(),
        )
