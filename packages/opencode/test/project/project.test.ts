...
// Project identity is pinned by a marker file inside the repo's git directory.
// Derived from Brand so these assertions follow a rename instead of checking a
// filename nothing writes any more.
const MARKER = Brand.projectMarker
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const opencodeFile = path.join(tmp, ".git", MARKER)
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", MARKER)
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, MARKER)
      const wrongCache = path.join(parentDir, ".git", MARKER)
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, MARKER)
      const cacheB = path.join(bareB, MARKER)
      const wrongCache = path.join(parentDir, ".git", MARKER)
