...
describe("ProviderTransform.smallOptions - caller-supplied variant", () => {
  // Variants generated by ProviderTransform.variants() come out weakest-first,
  // which is what makes the positional fallback below safe. A hand-written
  // config has no such guarantee: one listing "max" first ran every small-model
  // call at max reasoning effort, which is why the variant is now passed in.
  const model = {
    providerID: "togetherai",
    api: { id: "thinkingmachines/Inkling-Small", npm: "@ai-sdk/togetherai" },
    variants: {
      max: { reasoningEffort: "max" },
      low: { reasoningEffort: "low" },
      none: { reasoningEffort: "none" },
  ...
  test("uses the variant it is given rather than the first declared", () => {
    expect(ProviderTransform.smallOptions(model, { reasoningEffort: "low" })).toEqual({ reasoningEffort: "low" })
  })

  test("an empty variant means no reasoning options, not the first declared", () => {
    expect(ProviderTransform.smallOptions(model, {})).toEqual({})
  })

  test("falls back to the first declared variant when the caller supplies none", () => {
    expect(ProviderTransform.smallOptions(model)).toEqual({ reasoningEffort: "max" })
  })
})
