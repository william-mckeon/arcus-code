...
// globalConfigFile() creates the highest-precedence Arcus name when no config
// exists yet. Derived from Brand so this tracks a future rename instead of
// asserting a literal that would silently go stale.
const GLOBAL_CONFIG_FILE = Brand.configFiles.toReversed()[0]
...
it.effect("creates global jsonc config with schema when no global configs exist", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const content = yield* FSUtil.use.readFileString(path.join(dir, GLOBAL_CONFIG_FILE))
    ...
    yield* withGlobalConfig({}, ({ dir }) =>
      withProcessEnv(
        ...
        Effect.gen(function* () {
          ...
          expect(yield* FSUtil.use.existsSafe(path.join(dir, GLOBAL_CONFIG_FILE))).toBe(false)
        }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
      ),
    )
  }),
