...
function globalConfigFile() {
  // First existing file wins, and when none exist candidates[0] is what gets
  // created — so Arcus names lead here, the opposite of the merge lists where
  // later entries override earlier ones.
  const candidates = [...Brand.configFiles.toReversed(), ...Brand.legacy.configFiles.toReversed(), "config.json"].map(
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      // Legacy names first so an Arcus-named file layered after them wins.
      for (const file of Brand.allConfigFiles) {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const dir of directories) {
          if (Brand.allConfigDirs.some((name) => dir.endsWith(name)) || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of Brand.allConfigFiles) {
              ...
              add: [
                {
                  ...
                  // Pin to our own version only if we publish one. Arcus builds
                  // carry versions like 0.0.0-dev-<timestamp> that exist on no
                  // registry, so pinning made every background install fail.
                  version: Brand.selfUpdate && !InstallationLocal ? InstallationVersion : undefined,
                },
              ],
            })
            .pipe(
