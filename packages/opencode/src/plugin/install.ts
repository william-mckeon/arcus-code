...
// Arcus name first so it wins when a project somehow has both, then the
// pre-rename name so an existing project keeps a single config location.
const CONFIG_DIRS = [Brand.configDir, Brand.legacy.configDir]
const SERVER_NAMES = [Brand.slug, Brand.legacy.slug]

async function patchDir(input: PatchInput, dep: PatchDeps) {
  if (input.global) return input.config ?? Global.Path.config
  const git = input.vcs === "git" && input.worktree !== "/"
  const root = git ? input.worktree : input.directory
  // Adopt whichever config directory the project already has. Creating the
  // Arcus one unconditionally would split a pre-rename project's plugins across
  // two directories, and only one of them would be the one they edit by hand.
  for (const dir of CONFIG_DIRS) {
    if (await dep.exists(path.join(root, dir))) return path.join(root, dir)
  }
  return path.join(root, Brand.configDir)
}

function patchNames(kind: Kind) {
  if (kind === "server") return SERVER_NAMES
  return ["tui"]
}
...
async function patchOne(dir: string, target: Target, spec: string, force: boolean, dep: PatchDeps): Promise<PatchOne> {
  ...
  // All accepted basenames, Arcus first. files[0] is what gets created when the
  // project has no config yet; an existing file anywhere in the list wins so we
  // edit it in place rather than writing a second config beside it.
