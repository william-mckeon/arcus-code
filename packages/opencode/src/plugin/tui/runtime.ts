...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir = Brand.allConfigDirs.includes(path.basename(source_dir))
      ? path.join(source_dir, "themes")
      : path.join(source_dir, Brand.configDir, "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, Brand.configDir, "tui.json"),
  }
}
