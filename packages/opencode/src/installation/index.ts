  ...
  Effect.gen(function* () {
    ...
    const result: Interface = {
      ...
      method: Effect.fn("Installation.method")(function* () {
        // Must stay in step with INSTALL_DIR in the root `install` script, and
        // with the PATH cleanup in cmd/uninstall.ts. Both names are checked so
        // upgrades from a pre-rename install are still detected as curl.
        if (Brand.allConfigDirs.some((dir) => process.execPath.includes(path.join(dir, "bin"))))
