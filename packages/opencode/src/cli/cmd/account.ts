...
// This is the opencode zen control plane, not branding. `account login` performs
// a real OAuth device flow against it, so it must keep resolving. Point it at an
// Arcus-owned host only once that host actually serves the same endpoints.
