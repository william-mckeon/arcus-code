...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info(
        `Remote MCP servers support OAuth by default. Add a remote server in ${Brand.configFiles[0]}:`,
      )
      prompts.log.info(`
  "mcp": {
    "my-server": {
...
async function resolveConfigPath(baseDir: string, global = false) {
  // First match wins and candidates[0] is what gets created, so Arcus names
  // lead here — the opposite of the merge lists, where later entries override
  // earlier ones. Legacy names still follow, so `mcp add` edits the config a
  // project already has instead of writing a second one beside it.
  const names = [...Brand.configFiles, ...Brand.legacy.configFiles]
  const dirs = [Brand.configDir, Brand.legacy.configDir]
  const candidates = names.map((file) => path.join(baseDir, file))

  if (!global) {
    for (const dir of dirs) {
      candidates.push(...names.map((file) => path.join(baseDir, dir, file)))
    }
  }
  ...
  // Nothing exists yet: create the highest-precedence Arcus name.
  return candidates[0]
}
