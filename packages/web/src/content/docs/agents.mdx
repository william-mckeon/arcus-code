...
OpenCode comes with three built-in primary agents, **Build**, **Collaborate**,
...
An agent that works **with** you rather than for you. It has exactly the same
tools as Build -- it is not restricted -- but before it changes anything it
presents what it found, lists every file it intends to update, add, or delete,
and waits for you to confirm.

It does this **even when it is certain**. That is the whole point: the pause is
a checkpoint, not a request for clarification. Where Build puts you at the end
reviewing a diff, Collaborate puts you at the moment of intent, before the work
is spent -- and correcting an intention costs a sentence where correcting a
wrong diff costs a revert.

Reading is never gated. It will search, map, and read as much as it needs
without asking; only changes are held.

This is not Plan mode. Plan cannot edit at all and produces a plan document.
Collaborate does the real work up front and then holds at the threshold.
...
`read`, `edit`, `glob`, `grep`, `tree`, `list`, `bash`, `task`, `external_directory`, `lsp`, and `skill` accept either a shorthand action (`"allow" | "ask" | "deny"`) or an object of glob/pattern → action for fine-grained control. The remaining keys accept the shorthand action only.
