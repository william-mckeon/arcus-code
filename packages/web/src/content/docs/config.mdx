...
The always-on checks are the two that need no citation to work: a claim that a path is missing or empty when it exists, and a claim that a build or test passed when nothing this turn ran one.

Three more are opt-in, because each can be noisy depending on how your model writes:

| Key | Flags |
| --- | --- |
| `check_paths` | A file path cited in the answer that does not exist and was never read |
