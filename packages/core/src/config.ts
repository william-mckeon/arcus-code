...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // Legacy names sort first, so when a project holds both opencode.json and
    // arcus-code.json the Arcus one is applied last and wins. `up()` walks
    // parents pushing targets in order and callers reverse within each level,
    // so `names.toReversed()` preserves that precedence there too.
    const names = Brand.allConfigFiles
    const directoryNames = Brand.allConfigDirs
