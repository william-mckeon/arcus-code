...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // Project identity is pinned by a marker inside the repo's git directory.
    // Read the pre-rename name too: without it `cached` returns undefined for
    // every existing repo and the ID falls through to remote-then-root-commit.
    // That usually recomputes the same ID, but any repo whose remote changed
    // after first use would silently adopt a new one and lose its history.
    ...
    const cached = Effect.fnUntraced(function* (dir: string) {
      return (
        (yield* read(path.join(dir, Brand.projectMarker))) ??
        (yield* read(path.join(dir, Brand.legacy.projectMarker)))
      )
    })
    ...
    const commit = Effect.fn("Project.commit")(function* (input: { store: AbsolutePath; id: ID }) {
      yield* fs.writeFileString(path.join(input.store, Brand.projectMarker), input.id).pipe(Effect.ignore)
    })
